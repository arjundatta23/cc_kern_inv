...
try:
	...
	dfile = sys.modules['__main__'].disp_ray
except AttributeError:
	# this is the 2D-scalar case
	assert sys.modules['__main__'].scalar==True

scalar = sys.modules['__main__'].scalar
...
class check_settings:
	...
	def __init__(self, dist_arr=None):

		# Check grid symmetry with respect to origin
		if (config.dom_geom.box_len/2)%dx != 0:
			raise Exception("Domain size (horizontal) incompatible with grid spacing; please check configuration file.")

		if dist_arr is None:
			self.dist_max = 2*config.rloc
		else:
		...
		self.fmax = sig_char.cf + 2.5*(config.sig_char.fsigma)
		if scalar:
			...
			try:
				self.groupspeed_cen = reou[np.searchsorted(reof,sig_char.cf)]
	...
	def time_series_length(self):
		...
		bandw = 4*config.sig_char.fsigma
		dur_TD = 6*(1./bandw)
		...
		tlen_branch = (sig_char.nsam * sig_char.dt)/2
...
class memory_reqt:
	...
	def __init__(self, nz):
		...
		self.Garr_storage = 16 * 1e-9 * (3 * 3 * config.sig_char.n_nn_fsam * self.nz * config.dom_geom.Youtmost.size * config.dom_geom.Xoutmost.size)
	...
	def struc_kern(self, kern_todo, kern_elast):
		...
		if kern_todo:
			...
			if kern_elast:
				for_div_vol = 16 * 1e-9 * (3 * config.sig_char.n_nn_fsam * self.nz * config.dom_geom.Youtmost.size * config.dom_geom.Xoutmost.size)
				for_grad_vol = 16 * 1e-9 * (2 * 3 * 3 * config.sig_char.n_nn_fsam * self.nz * config.dom_geom.Youtmost.size * config.dom_geom.Xoutmost.size)
	...
	def source_kern(self, nrecs):
		...
		for_wforms = 16 * sig_char.nsam * (nrecs**2) * 1e-9
