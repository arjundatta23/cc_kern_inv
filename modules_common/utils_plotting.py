...
# global variables: from main (calling) program
# recA = sys.modules['__main__'].loc_A
# recB = sys.modules['__main__'].loc_B

scal_eq = sys.modules['__main__'].user_choices['scalar']
...
def plot_section_vertical_inline(arrin, yloc):
    ...
    yindex = np.searchsorted(dg.Y, yloc)
    try:
        yinfo = "Y-position: %.1f" %(dg.Y[yindex])
    except IndexError:
...
def plot_section_vertical_crossline(arrin, xloc):
    ...
    try:
        vgx, vgzx = np.meshgrid(dg.X,Z)
        vgy, vgzy = np.meshgrid(dg.Y,Z)
    except NameError:
        pass

    xindex = np.searchsorted(dg.X, xloc)
    try:
        xinfo = "X-position: %.1f" %(dg.X[xindex])
    except IndexError:
...
def plot_section_horizontal(inplist, Z, depth, heading, ptype):
    ...
    for p in range(nplots):
        if nplots>1:
            ax_use = ax[p]
        cax=ax_use.pcolor(dg.gx, dg.gy, plotlist[p], vmin=min_val, vmax=max_val, cmap=plt.cm.jet)
        # cax=ax_use.pcolor(dg.gx, dg.gy, plotlist[p], vmin=-2e-5, vmax=4e-5, cmap=plt.cm.jet)
    ...
    # fig2=plt.figure()
    # ax2=fig2.add_subplot(111)
    # cax2=ax2.pcolor(dg.gx,dg.gy,plotlist[-1])#,vmin=min_val, vmax=max_val, cmap=plt.cm.jet)
...
def plot_source_spectrum():
	...
	axs.plot(np.fft.fftshift(config.sig_char.fhz),np.fft.fftshift(config.sig_char.pow_spec_sources),'-o')
...
def plot_wforms_TD(cc, **more_wforms):
    ...
    axas.plot(config.sig_char.tt,cc,label='$C$')

    if len(more_wforms)==1:
        # this would be the adjoint_source
        adjsrc = more_wforms['asrc']
        axas.plot(config.sig_char.tt,adjsrc*1e-13,label='ASRC*1e-16')
        ...
        axas.plot(config.sig_char.tt,cc_der_TD,label='$\dot{C}$ TD calculation')
        axas.plot(config.sig_char.tt,cc_der_FD,label='$\dot{C}$ FD calculation')
...
def plot_domain():
    ...
    figd=plt.figure()
    axd=figd.add_subplot(111,aspect='equal')
    dummy = np.ones((dg.Y.size,dg.X.size))
    caxd=axd.pcolor(dg.gx, dg.gy, dummy, cmap=plt.cm.Greys)
    axd.set_xlabel('X [km]', fontsize=14)
    axd.set_ylabel('Y [km]', fontsize=14)

    # rlocsx = irlocsx * dxy
    # rlocsy = irlocsy * dxy

    axd.plot(-162.5, 0, 'kv', markersize=12)
    axd.plot(162.5, 0, 'kv', markersize=12)
...
def plot_horizontal_outmost_domain(sample_GF, inx, iny):
    ...
    # getind_x = lambda m: np.searchsorted(dg.Xoutmost, m)
    # getind_y = lambda m: np.searchsorted(dg.Youtmost, m)
    getind_x = lambda m: np.searchsorted(dg.X3, m)
    getind_y = lambda m: np.searchsorted(dg.Y3, m)
    ...
    caxo=axo.pcolor(dg.gx3, dg.gy3, sample_GF.real)
    ...
    for pair in range(xind.shape[0]):
        ...
        boxstart = np.array(list(map(lambda m: m-dg.ngp_hlen, mirror_ind)))
        boxend = boxstart + (2*dg.ngp_hlen)

        print("Box start in gridpts: ", boxstart)
        print("Box end in gridpts: ", boxend)

        # finally convert to plot coordinates for plotting lines
        plot_coordinates = lambda m: float(m)/(dg.X3.size-1)
        ...
        axo.plot(slocsx[pair], slocsy[pair], 'w^', markerfacecolor="None")
        axo.plot(mirror_x[pair], mirror_y[pair], 'wo', markerfacecolor="None")
        axo.plot(0,0,'ro')

        bspc_y = float(config.syn_data.ofac-1)/(2*config.syn_data.ofac)
        bspc_x = float( (config.syn_data.ofac-1)*dg.ngp_hlen ) / (2*config.syn_data.ofac*dg.ngp_hlen)
        #bspc -> box_start_plot_coordinates

        hlen = dg.box_len/2

        axo.axvline(-hlen,ymin=bspc_y,ymax=1-bspc_y)
        axo.axvline(hlen,ymin=bspc_y,ymax=1-bspc_y)
        axo.axhline(-hlen,xmin=bspc_x,xmax=1-bspc_x)
        axo.axhline(hlen,xmin=bspc_x,xmax=1-bspc_x)

        for e in boxedge:
        	#axo.axvline(dg.Xoutmost2[boxedge[e][0]],color='w',ls='--')
        	axo.axhline(dg.Y3[boxedge[e][1]],xmin=start_pc[0],xmax=end_pc[0],ls='--')
        	axo.axvline(dg.X3[boxedge[e][0]],ymin=start_pc[1],ymax=end_pc[1],ls='--')
